...
sd_16s_aug <- sd_16s %>%
  left_join(
    metadata %>% dplyr::select(01:33,40:68) %>%
      dplyr::select(-Cruise, -Date, -Area, -StationID, -Depth, -Water_mass),
    by = "ps_id"
  )
...
env <- metadata %>%
  dplyr::select(Depth, Salinity, Temp, NO3, PO4, SiO2, Fluorescence_Chla) %>%
  drop_na()
...
```{r, message=FALSE, warning=FALSE}
env <- metadata %>%
  dplyr::select(Depth, Temp, Salinity, NO3,NO2, SiO2, PO4, DOC, Chla, O2_sat, a350, C1, C2, C3,C4,C5) %>% tidyr::drop_na()
