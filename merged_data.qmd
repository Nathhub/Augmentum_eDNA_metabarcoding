...
metadata <- read_csv("data/Metadata_Augmentum.csv") %>% select(01:24)
...
We can test it with vif.cca(). VIF = Variance Inflation Factor
...
## PCA analysis with environmental conditions

Note:
To identify specific taxa (at the genus level) that are being driven by environmental conditions, I could use:
